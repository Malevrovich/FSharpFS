...
type BitmapPersisterPool(
        filename: string, 
        initialState: Bitmap, 
        allocator: BitmapAllocator, 
        baseOffset: uint32) =
    let filePool = new ObjectPool<FileStream>(
        16u, 
...
let deserializeAllocator (blockCount: uint) (filename: string) (stream: Stream) = 
    ...
    let bytes = Seq.init (bitmapLen (int blockCount)) (fun _ -> 0UL) |> Seq.map (BitConverter.GetBytes) |> Array.concat
    stream.Read(span bytes) |> ignore
    
    let state = bytes |> Seq.chunkBySize sizeof<uint64> |> Seq.map BitConverter.ToUInt64 |> Array.ofSeq
