...
let mdAllocator fs = fs.MetadataStorage.PersistableAllocator.Allocator
let strAllocator fs = fs.StringStorage.PersistableAllocator.Allocator
let mdAllocatorPersister fs = fs.MetadataStorage.PersistableAllocator.Persister
let strAllocatorPersister fs = fs.StringStorage.PersistableAllocator.Persister
...
let rec tryApplyTreeOperation operation fileTreeSelector (filesystem: FileSystem) = 
  result {
    ...
    if (LanguagePrimitives.PhysicalEquality 
          (Interlocked.CompareExchange(&filesystem.FileTree, newTree, tree)) 
          tree) 
      then 
        return res
      else 
...
let createDirectory (path: string) (uid: uint) (gid: uint) (mode: mode_t) (filesystem: FileSystem) = 
  ...
  let revertDir = (fun () -> filesystem |> tryApplyTreeOperation removeDir snd |> ignore)
  ...
  |> bindUndoAndResult (
    fun (parentAddr, _) -> 
      option {
        ...
        return Ok(
          [revert],
          (parentAddr, {BlockId = strAddr})
        )
      } |> Option.defaultValue (Error LibC.ENOSPC)
  )
  |> bindUndoAndResult (
    fun (parentAddr, strAddr) -> 
      option {
        ...
        return Ok(
          [revert],
          (parentAddr, strAddr, {BlockId = fileAddr})
        )
      } |> Option.defaultValue (Error LibC.ENOSPC)
  )
  |> bindUndoAndResult (
    fun (parentAddr, strAddr, fileAddr) ->
      let dirMD = {
            Common = {
      ...
      |> Result.map (
        fun res -> ([revertStrAlloc; revertNodeAlloc], res) 
      )
  )
...
let removeDirectory (path: string) (filesystem: FileSystem) = 
  ...
  let updateOp node = 
    match node with 
    | DirectoryNode(dirMD) 
        when (snd dirMD.Common.Name) = filename -> 
          if dirMD.Content.IsEmpty 
          then 
...
let chown (path: string) (uid: uint) (gid: uint) (filesystem: FileSystem) = 
  ...
  let updateOwner node = 
    match node with
    | FileNode(fileMD) -> Ok <| FileNode { fileMD with Common.Uid = uid; Common.Gid = gid }
    | DirectoryNode(dirMD) -> Ok <| DirectoryNode { dirMD with Common.Uid = uid; Common.Gid = gid }
    | FillerNode(_) -> Error LibC.ENOENT

  let updateOp = tryUpdateNodeAt dirPath filename updateOwner

  filesystem
  |> tryApplyTreeOperation updateOp snd 
  |> Result.map ignore
